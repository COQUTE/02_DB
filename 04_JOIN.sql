...
-- 두 테이브의 지정하는 컬럼값이 일치하지 않는 행도 조인에 포함 시킴
...
-- ANSI 표준
SELECT E.EMP_ID AS 사번, E.EMP_NAME AS 사원이름,
NVL(E.MANAGER_ID, '없음') AS "사수의 사번",
NVL(M.EMP_NAME, '-') AS "사수의 이름"
FROM EMPLOYEE E
...
FROM EMPLOYEE E
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE) -- EMPLOYEE + JOB
...
조회하시오.*/
SELECT EMP_NO, JOB_NAME, DEPT_CODE, DEPT_TITLE
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
...
SELECT EMP_NAME, JOB_NAME, SALARY, (SALARY * 12) * (1 + NVL(BONUS, 0)) AS "ANNUAL SALARY"
FROM EMPLOYEE E
JOIN JOB USING(JOB_CODE)
JOIN SAL_GRADE S ON (E.SAL_LEVEL = S.SAL_LEVEL AND SALARY > MIN_SAL);
