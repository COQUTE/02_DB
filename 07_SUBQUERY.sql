 ...
 * 
 * - IN / NOT IN : 여러 개의 결과값 중에서 한개라도 일치하는 값이 있다면
...
SELECT EMP_NO, EMP_NAME, DEPT_CODE, JOB_CODE, JOB_NAME
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING (JOB_CODE)
WHERE EMP_NAME != '노옹철'
AND (DEPT_CODE, JOB_CODE) IN (SELECT DEPT_CODE, JOB_CODE
															FROM EMPLOYEE
															JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
															JOIN JOB USING (JOB_CODE)
															WHERE EMP_NAME = '노옹철');
...
-- 상관쿼리
SELECT EMP_NO, EMP_NAME, DEPT_TITLE, MANAGER_ID
FROM EMPLOYEE MAIN
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
WHERE MANAGER_ID = (SELECT EMP_ID FROM EMPLOYEE SUB
...
FROM (SELECT EMP_NAME, SALARY
			FROM EMPLOYEE
			ORDER BY SALARY DESC) -- 해석순위 1위 FROM 절에서 전체 직원의 SALARY 내림 차순 정렬
...
FROM (SELECT DEPT_ID, DEPT_TITLE, CEIL(AVG(SALARY)) 평균급여
			FROM EMPLOYEE
			LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
