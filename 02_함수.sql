...
-- 그룹 함수 : N개의 값을 읽어서 연산 후 1개의 결과를 반환(합계, 평군, 최대, 최소)

-- 함수는 SELECT 문의
-- SELECT절, SHERE절, ORDER BY절, GROUP BY절, HAVING 절 사용 가능
...
-- SYSTIMESTAMP : SYSDATE + MS 단위 추기
SELECT SYSTIMESTAMP FROM DUAL; -- 2025-03-10 12:12:11.089 +0900

-- MONTHS_BETWEEN(날짜, 날자) : 두 날짜의 개월 수 차이 반환
SELECT ABS(ROUND(MONTHS_BETWEEN(SYSDATE, '2025-07-22'), 3)) || '개월' AS "수강 기간(개월)"
FROM DUAL; -- 4.371

-- EMPLOYEE 테이블에서
-- 사원의 이름, 입사일, 근무한 개월수, 근무 년차 조회
SELECT EMP_NAME, HIRE_DATE,
CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) || '개월' AS "근무한 개월수",
...
-- 0 : 숫자 한칸을 의미, 여러개 작성시 오름쪽 정렬 + 빈칸 0 추가
...
SELECT EMP_NAME, SALARY, NVL(BONUS, 0), SALARY * NVL(BONUS, 0.1)
FROM EMPLOYEE;
