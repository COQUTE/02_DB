...
--ALTER TABLE DEPT_COPY
--ADD CONSTRAINT DEPT_COPY_TITLE_NN CHECK(DEPT_TITLE IS NOT NULL);
-- ORA-00904: : 부적합한 식별자
...
SELECT * FROM DEPT_COPY;
--> 컬럼이 생성되면서 DEFAULT 값이 자동 삽입되어 있음
...
-- DEPT_COPY 테입블에 PK 추가 (컬럼 : DEPT_ID, 제약조건명 : D_COPY_PK)
...
CREATE TABLE TB2(
	...
	TB2_COL NUMBER REFERENCES TB1 -- FK 제약조건
); -- 자식 테이블
